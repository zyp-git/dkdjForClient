...
@interface BindShopCardViewController ()

@end

@implementation BindShopCardViewController
...
- (void)save:(id)sender
{
    ...
    url = [NSURL URLWithString:GetBindShopCardURLString(uduserid,self._userName, self._password)];
...
-(void) GetResult:(ASIHTTPRequest *)request
{
    ...
    if ([code isEqualToString:@"200"]) {
        NSString *msg = [[NSString alloc] initWithFormat:@"绑定成功:面值:%@,剩余金额:%@",[dic objectForKey:@"point"],[dic objectForKey:@"cmoney"] ];
        [tooles MsgBox:msg];
    }
    else
    {
