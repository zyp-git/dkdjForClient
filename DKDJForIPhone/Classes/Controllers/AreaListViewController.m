...
- (void)areaDidReceive:(TwitterClient*)client obj:(NSObject*)obj
{
    ...
    for (int i = 0; i < [ary count]; ++i) {
        ...
        AreaMode *model = [[AreaMode alloc] initWithJsonDictionary:dic];
...
-(void)getAddrList
{
    ...
    NSDictionary* param = [[NSDictionary alloc] initWithObjectsAndKeys:nil, pageindex,@"pageindex",@"8",@"pagesize",self.cTypeID ,@"cid", nil];
    
    [twitterClient getAreaList:param];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath 
{
    ...
    if (indexPath.row < [area count]) {
        ...
        if (mode != nil) {
            ...
            [defaults setObject:mode.name forKey:@"AreaName"];
            [defaults setObject:mode.aID forKey:@"AreaID"];
            ...
            if (isReturn) {
                ...
                ByAreaListViewController *viewController = [[[ByAreaListViewController alloc] initWithDefaultReturn:mode.aID] autorelease];
                viewController.title =@"选择商圈";
                [self.navigationController pushViewController:viewController animated:YES];
                
            }else{
                ByAreaListViewController *viewController = [[[ByAreaListViewController alloc] initWithDefault:mode.aID] autorelease];
                viewController.title =@"选择商圈";
