...
@implementation PayByShopCardViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"选择店铺券";
    ...
    if ([uduserid intValue] > 0 )
    {
        ...
        NSString *shopid = [[NSUserDefaults standardUserDefaults] stringForKey:@"shopcartshopid"];
        
        //state 0 获取有余额的  其他值为获取所有
        NSDictionary* param = [[NSDictionary alloc] initWithObjectsAndKeys:pageindex,@"pageindex",@"8",@"pagesize",uduserid,@"userid",@"0",@"state", shopid, @"shopid",nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == [orders count]) {
        ...
        telLabel.text = [[NSString alloc] initWithFormat:@"面值:%.1f",[ordermodel.cmoney floatValue]];
        
        UILabel *timeLabel = (UILabel *)[cell.contentView viewWithTag:3];
        timeLabel.text = [[NSString alloc] initWithFormat:@""];
        
        //时间限制 不限制金额
        if( [ordermodel.timelimity isEqualToString:@"0"])
        {
            timeLabel.text = [[NSString alloc] initWithFormat:@"限制:%@-%@", ordermodel.starttime, ordermodel.endtime];
        }
        
        //时间不限制 限制金额
        if([ordermodel.moneylimity isEqualToString:@"0"])
        {
            timeLabel.text = [[NSString alloc] initWithFormat:@"订单满%@元使用",ordermodel.moneyline];
        }
        
        //时间限制 限制金额
        if([[ordermodel timelimity] isEqualToString:@"0"] && [[ordermodel moneylimity] isEqualToString:@"0"])
        {
            timeLabel.text = [[NSString alloc] initWithFormat:@"%@-%@,订单满%@元使用", ordermodel.starttime,ordermodel.endtime,ordermodel.moneyline];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        //设置选中值
        ShopCardModel* cardItem = [orders objectAtIndex:indexPath.row];
...
- (void)setEditing:(BOOL)editting animated:(BOOL)animated
{
	if (!editting)
	{
		for (ShopCardModel* item in orders)
		{
			item.checked = NO;
		}
	}
...
-(void)done:(id)sender{
    ...
    for (ShopCardModel *model in orders)
    {
        if(model.checked)
        {
            self.shopcardid = model.ckey;
        }
    }
    
    [self.delegate SelectShopCardViewControllerChanged:self.shopcardid];
