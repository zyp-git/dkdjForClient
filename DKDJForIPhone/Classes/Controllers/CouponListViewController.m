...
- (void)viewDidLoad
{
    ...
    NSLog(@"viewDidLoad");
    imageDownload = [[CachedDownloadManager alloc] initWitchReadDic:11 Delegate:self];
...
-(void) GetData:(BOOL)isShowProcess
{
    ...
    [twitterClient getCouponListByShopId:param];
    if (isShowProcess) {
...
-(void)cachedDownloadFinish:(NSMutableArray *)arry start:(int)index dowType:(int)type{
    ...
    for (int i = index; i < [arry count]; i++) {
        ...
        if (task.index >= 0 && task.index < [activitys count]) {
            model1 = (CouponModel *)[activitys objectAtIndex:task.index];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView.tag == 1) {
            ...
            if (cell == nil) {
                ...
                //1. 名称
                UIView *view = [[UIView alloc] initWithFrame:CGRectMake(100.0, 10.0, 225, 55)];
            ...
            CouponModel *foodmodel = [activitys objectAtIndex:row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag == 1) {
        ...
        return 65;//(indexPath.row == [foods count]) ? 60 : 40;
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag == 1) {
        ...
        if (indexPath.row < [activitys count]) {
            ...
            CouponDetailViewController *controller = [[[CouponDetailViewController alloc] initWithFood:(CouponModel *)[activitys objectAtIndex:indexPath.row] ShowGoToShop:YES]autorelease];
