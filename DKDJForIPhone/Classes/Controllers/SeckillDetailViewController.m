...
@implementation SeckillDetailViewController
...
- (void)GetShopDetail:(NSString*)groupId
{
    ...
    [twitterClient getSeckillDetailById:GroupId];
...
- (void)getShopDetailDidSucceed:(TwitterClient*)sender obj:(NSObject*)obj;
{
    ...
    self.title = [self.dic objectForKey:@"foodname"];
    self.price = [self.dic objectForKey:@"newprice"];
    
    self.shopid = [self.dic objectForKey:@"shopid"];
    ...
    //if( ![[self.dic objectForKey:@"openstate"] isEqualToString:@"1"])
    {
        UIBarButtonItem *saveButton = (UIBarButtonItem *)[self.view viewWithTag:8001];
        saveButton.title = @"已经结束";
    ...
    [defaults setObject:[self.dic objectForKey:@"sqid"]  forKey:@"shopcartsentmoney"];
    [defaults setObject:@"1" forKey:@"shopcartstatus"];
    [defaults setObject:@"2" forKey:@"ordertype"];
    [defaults synchronize];
    //---------------------
    
    NSLog(@"shopcartsentmoney:%@",[self.dic objectForKey:@"sqid"]);
...
- (void)gotoOrder
{
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:@"您的购物车中有其商品，需要清空购物车后才能秒杀？" delegate:self cancelButtonTitle:@"取消." otherButtonTitles:@"清空",nil];
...
-(void)AddtoCart
{
    ...
    //如果数量大于0 不存在shopcart中 加入购物车
    //如果数量等于0 存在shopcart中 删除购车中的餐品
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if( indexPath.section == 1 )
    {
        ...
        /*
         {"fID":"21","foodname":"素三样","statestr":"立即秒杀","openstate":"1","timestr":"正在秒杀中...","togoname":"山东人家","oldprice":"8.0","newprice":"5.0","sqid":"2","cnum":"4","Foodid":"74527"}
         */
        switch (row) {
            case 0:
            {
            ...
            {
                // 該行要顯示的內容
                NSString *desc = @"1、秒杀前请先登录并检查您的收货地址\n2、秒杀商品数量有限，以用户秒杀订单的提交时间为依据， 秒杀品为N份，则由前N位提交订单的用户获得。\n3、秒杀商品 仅限当日配送.\n4、本站保留对此活动的最终解释权。";
                
                cell.textLabel.text = @"秒杀规则";
    ...
    else
    {
        ...
        NSString *prices = [[NSString alloc] initWithFormat:@"原价:%@ 现价:%@ 配送费:%@",[self.dic objectForKey:@"oldprice"],[self.dic objectForKey:@"newprice"],[self.dic objectForKey:@"sqid"]];
        
        valueLabel1.textAlignment = NSTextAlignmentLeft;
        valueLabel1.text = [self.dic objectForKey:@"timestr"];
        ...
        valueLabel3.text = [self.dic objectForKey:@"cnum"];
