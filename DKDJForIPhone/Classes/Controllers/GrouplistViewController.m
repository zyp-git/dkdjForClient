...
-(void)getShopList
{
    ...
    [twitterClient getGroupListByBid:param];
...
- (void)shopsDidReceive:(TwitterClient*)client obj:(NSObject*)obj
{
    ...
    [loadCell.spinner stopAnimating];//无效 TODO
    ...
    for (int i = 0; i < [ary count]; ++i) {
        ...
        GroupListModel *model = [[GroupListModel alloc] initWithJsonDictionary:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == [shops count]) {
        ...
        GroupListCell *cell =(GroupListCell *) [tableView dequeueReusableCellWithIdentifier:CellTableIdentifier];
        
        if (cell == nil) {
            
            cell = [[[GroupListCell alloc] initWithStyle:UITableViewCellStyleDefault
        ...
        GroupListModel *shopmodel = [shops objectAtIndex:indexPath.row];
        ...
        NSString *price = [[NSString alloc] initWithFormat:@"原价:%@ 折扣:%@ 现价:%@ 运费:%@",shopmodel.price, shopmodel.nowdis, shopmodel.discount, shopmodel.sentmoney];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == [shops count])
    {
        ...
        [loadCell.spinner startAnimating];//有效
        
        [self getShopList];
    }
    else
    {
        GroupListModel *model = [shops objectAtIndex:indexPath.row];
        
        GroupDetailViewController *shopdetail = [[[GroupDetailViewController alloc] initWithGroupId:model.dataid] autorelease];
