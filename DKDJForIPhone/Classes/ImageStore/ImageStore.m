...
{   //delegate 是 ImageStoreReceiver
    ...
    UIImage *image = [images objectForKey:url];//检查是否存在这个图片
    if (image) {
        return image;
    }
	...
	//远程下载
    ImageDownloader *dl = [pending objectForKey:url];//pending   NSMutableDictionary
    if (dl == nil) {
        dl = [[[ImageDownloader alloc] initWithDelegate:self] autorelease];
        [pending setObject:dl forKey:url];
    }

    NSMutableArray *arr = [delegates objectForKey:url];//delegates NSMutableDictionary
    if (arr) {
    ...
    if ([pending count] <= MAX_CONNECTION && dl.requestURL == nil) {
        [dl get:url];//开始下载 下载后会自动更新View中的图片
    }
    //默认先返回默认图片
    ...
    if (image) {
        ...
        if (arr) {
            for (id delegate in arr) {
                if ([delegate respondsToSelector:@selector(profileImageDidGetNewImage:)]) {//profileImageDidGetNewImage 在ImageStoreReceiver
