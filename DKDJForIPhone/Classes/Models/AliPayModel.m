...
-(void)dealloc
{
    [self.partner release];
    [self.seller release];
    [self.tradeNO release];
    [self.productName release];
    [self.productDescription release];
    [self.amount release];
    [self.notifyURL release];
    
    [self.service release];
    [self.paymentType release];
    [self.inputCharset release];
    [self.itBPay release];
    [self.showUrl release];
    
    
    [self.rsaDate release];//可选
    [self.appID release];//可选
    [super dealloc];
}
@end
